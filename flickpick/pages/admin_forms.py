...
class WidgetForm(FileRecoveryForm):
    ...
    class Meta:
        widgets = {
            'source_genre': FilteredSelect(),
            'source_director': FilteredSelect(),
            'source_actor': FilteredSelect(),
            'source_year': FilteredSelect(),
        }
        fields = '__all__'
...
class PageLinkFormMixin(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['link'].widget.sections = [
            ...
            for model in [pages.models.Page, Movie, Genre, Person]
        ]
        if self.instance:
...
class AdGroupWidgetForm(AbstractGroupWidgetForm):
    item_model = pages.models.AdGroupItem


class AdCarouselWidgetForm(AbstractGroupWidgetForm):
    item_model = pages.models.AdCarouselItem


class MovieFocusWidgetForm(WidgetForm):
    item_model = pages.models.MovieFocusWidget

    class Meta:
        fields = '__all__'
        widgets = {
            'movie': FilteredSelect(),
        }


class CatalogGroupWidgetForm(WidgetForm):
    source = GenericLookupField(required=False)

    def __init__(self, *args, **kwargs):
        super(CatalogGroupWidgetForm, self).__init__(*args, **kwargs)
        self.fields['source'].widget.sections = [
            ContentType.objects.get_for_model(model).id
            for model in self._meta.model().source_types()
        ]
    ...
    def clean(self):
        ...
        return super(CatalogGroupWidgetForm, self).clean()

    def save(self, commit=True):
        self.instance.source = self.cleaned_data['source']
        return super(CatalogGroupWidgetForm, self).save(commit)
...
def get_widget_form(widget_type=None, prefix='__prefix__', inline=True, data=None, *args, **kwargs):
    ...
    registry = {
        pages.models.AdGroupWidget: AdGroupWidgetForm,
        pages.models.AdCarouselWidget: AdCarouselWidgetForm,
        pages.models.MovieFocusWidget: MovieFocusWidgetForm,
    }
    if widget_type in registry:
        ...
        elif issubclass(widget_type, pages.models.CatalogGroupWidget) and not widget_type.multi_source:
            BaseForm = CatalogGroupWidgetForm
        else:
            BaseForm = WidgetForm
